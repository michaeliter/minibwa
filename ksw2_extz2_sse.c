...
void mb_ksw_extz2_sse(void *km, int qlen, const uint8_t *query, int tlen, const uint8_t *target, int8_t m, const int8_t *mat, int8_t q, int8_t e, int w, int zdrop, int end_bonus, int flag, ksw_extz_t *ez)
{
...
#define __dp_code_block2 \
	...
	mem = (uint8_t*)mb_kcalloc(km, tlen_ * 6 + qlen_ + 1, 16);
	...
	for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
		...
		if (!approx_max) { // find the exact max with a 32-bit score array
			...
			if (mb_ksw_apply_zdrop(ez, 1, max_H, r, max_t, zdrop, e)) break;
			...
			if (r > 0) {
				...
				if ((flag & KSW_EZ_APPROX_DROP) && mb_ksw_apply_zdrop(ez, 1, H0, r, last_H0_t, zdrop, e)) break;
