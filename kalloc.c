...
static header_t *morecore(kmem_t *km, size_t nu)
{
	...
	q = (header_t*)mb_kmalloc(km->par, bytes);
	...
	*p = nu - 1; /* the size of the free block; -1 because the first unit is used for the core header */
	mb_kfree(km, p + 1); /* initialize the new "core"; NB: the core header is not looped. */
	return km->loop_head;
}

void mb_kfree(void *_km, void *ap) /* mb_kfree() also adds a new core to the circular list */
{
	header_t *p, *q;
	kmem_t *km = (kmem_t*)_km;
	...
	if (p + p->size == q->ptr) { /* two adjacent blocks, merge p and q->ptr (the 2nd and 4th cases) */
		...
		panic("[mb_kfree] The end of the allocated block enters a free block.");
	...
	if (q + q->size == p) { /* two adjacent blocks, merge q and p (the other two cases) */
		...
		panic("[mb_kfree] The end of a free block enters the allocated block.");
	} else km->loop_head = p, q->ptr = p; /* in two cores, cannot be merged; create a new block in the list */
}

void *mb_kmalloc(void *_km, size_t n_bytes)
{
	kmem_t *km = (kmem_t*)_km;
	size_t n_units;
	header_t *p, *q;
	...
	if (!(q = km->loop_head)) /* the first time when mb_kmalloc() is called, intialize it */
...
void *mb_kcalloc(void *_km, size_t count, size_t size)
{
	kmem_t *km = (kmem_t*)_km;
	void *p;
	if (size == 0 || count == 0) return 0;
	if (km == NULL) return calloc(count, size);
	p = mb_kmalloc(km, count * size);
	memset(p, 0, count * size);
	return p;
}

void *mb_krealloc(void *_km, void *ap, size_t n_bytes) // TODO: this can be made more efficient in principle
{
	kmem_t *km = (kmem_t*)_km;
	size_t cap, *p, *q;

	if (n_bytes == 0) {
		mb_kfree(km, ap); return 0;
	}
	if (km == NULL) return realloc(ap, n_bytes);
	if (ap == NULL) return mb_kmalloc(km, n_bytes);
	p = (size_t*)ap - 1;
...
void mb_km_stat(const void *_km, km_stat_t *s)
{
	...
	for (p = km->loop_head;; p = p->ptr) {
		...
		if (p->ptr > p && p + p->size > p->ptr)
			panic("[mb_km_stat] The end of a free block enters another free block.");
...
void mb_km_stat_print(const void *km)
{
	km_stat_t st;
	mb_km_stat(km, &st);
	fprintf(stderr, "[mb_km_stat] cap=%ld, avail=%ld, largest=%ld, n_core=%ld, n_block=%ld\n",
